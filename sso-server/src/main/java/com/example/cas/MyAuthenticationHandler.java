...
public class MyAuthenticationHandler extends AbstractJdbcUsernamePasswordAuthenticationHandler {
    ...
    protected HandlerResult authenticateUsernamePasswordInternal(final UsernamePasswordCredential credential, final String originalPassword)
        ...
        //判断账号是否被激活
	       try {
	    	   String state="";
	    	   state=""+getJdbcTemplate().queryForObject("select state from user where email='"+username+"'", Integer.class);
	    	   if("".equals(state)) {// Account Not Found
	    		   throw new AccountNotFoundException(username + " not found with SQL query");
	    	   }
	    	   
	    	   if("0".equals(state)) {// Account not be activated.
	    		   //LOGGER.debug("Account="+username+" state is 0");
	    		   throw new AccountNotActivatedException("Account not be activated");
	    	   }
